    ...
    public partial class PEPParser : IDisposable
    {
        ...
        private unsafe ParserError ProcessPE(string filename)
        {
            ...
            try
            {
                ...
                peData.NT_HEADER_FOA = _DOS_HEADER.e_lfanew;

                uint ntsig = (uint)_viewAccessor.ReadInt32((long)peData.NT_HEADER_FOA);
                ...
                ushort exe32bit = (ushort)_viewAccessor.ReadInt16((long)(peData.NT_HEADER_FOA + 4 + IMAGE_SIZEOF_FILE_HEADER));
                switch (exe32bit)
                {
            ...
            Process32Bit:
                ...
                _viewAccessor.ReadArray((long)peData.NT_HEADER_FOA, buffer, 0, _len);
            ...
            Process64Bit:
                ...
                _viewAccessor.ReadArray((long)peData.NT_HEADER_FOA, buffer0, 0, len0);
            ...
            CommonProcess:
                ...
                if (idd.VirtualAddress != 0 && idd.Size != 0)
                {
                    ...
                    for (int i = 0; i < len; i++)
                    {
                        ...
                        {
                            _viewAccessor.Read((long)(peData.RESOURCE_DIR_FOA + pirde[i].NameOffset), out stringu);
                            res.ResType = (ResourceType)pirde[i].Id;
                            res.Name = GetStringUFromAddr((long)(peData.RESOURCE_DIR_FOA + pirde[i].NameOffset + sizeof(short)), stringu.Length);
                            res.ID = null;
                        }
                        else
                        {
                        ...
                        if (pirde[i].DataIsDirectory)
                        {
                            ...
                            for (int ii = 0; ii < len1; ii++) //第二层
                            {
                                ...
                                {
                                    _viewAccessor.Read((long)(peData.RESOURCE_DIR_FOA + pirde2[ii].NameOffset), out stringu);
                                    res.ResName = GetStringUFromAddr((long)(peData.RESOURCE_DIR_FOA +
                                ...
                                ulong irdebase3 = peData.RESOURCE_DIR_FOA + pirde2[ii].OffsetToDirectory;
                                ...
                                {
                                    _viewAccessor.Read((long)(peData.RESOURCE_DIR_FOA + pirde3[iii].OffsetToData), out irdata);
                        ...
                        else
                        {
                            ...
                            ulong irdebase3 = peData.RESOURCE_DIR_FOA + pirde[i].OffsetToDirectory;
                            _viewAccessor.Read((long)irdebase3, out ird3);
                            irdebase3 += (uint)sizeof(IMAGE_RESOURCE_DIRECTORY);
                ...
                if (idd.VirtualAddress != 0 && idd.Size != 0)
                {
                    ...
                    {
                        peData.MetaData_FOA = RVA2FOA(_COR20_HEADER.MetaData.VirtualAddress).Value;
                        netMeta.STORAGESIGNATURE = new STORAGESIGNATURE();
                        netMeta.STORAGEHEADER = new STORAGEHEADER();

                        _viewAccessor.Read((long)peData.MetaData_FOA, out netMeta.STORAGESIGNATURE);
                        ulong pos = peData.MetaData_FOA + (uint)sizeof(STORAGESIGNATURE);
